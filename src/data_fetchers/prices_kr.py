...
class KoreanPriceFetcher(BaseFetcher):
    ...
    def fetch_daily(self, start_date: str, end_date: str, limit: int = None) -> pd.DataFrame:
        """한국 KOSPI 종목 일별 종가 및 OHLCV 수집 (yfinance 벌크 쿼리 + 스태킹 + 로컬 캐시 최적화)."""
        cached = self.load("prices_kr_daily_raw")
        if cached is not None:
            print("Loaded KR daily prices from cache.")
            return cached
            
        print("Fetching KOSPI tickers for daily data...")
        ...
        # 2. yfinance 벌크 쿼리
        print(f"Downloading Korean daily prices in bulk for {len(tickers)} KOSPI tickers via yfinance...")
        fallback_tickers = [f"{t}.KS" for t in tickers]
        ...
        for i, chunk in enumerate(chunks):
            try:
                print(f"Downloading KR daily chunk {i+1}/{len(chunks)} ({len(chunk)} tickers)...")
                df = yf.download(chunk, start=start_date, end=end_date, progress=False, auto_adjust=True)
                if df.empty:
                    continue
                ...
                print(f"Error in KR yfinance daily chunk {i+1}: {e}")
                continue
                
        if not all_yf_dfs:
            print("Warning: No KR daily price data fetched from yfinance.")
            return pd.DataFrame()
        ...
        # 필요한 컬럼만 선택
        full_yf = full_yf[['ticker', 'country', 'currency', 'date', 'open', 'high', 'low', 'close', 'volume']]
        
        # 타입 변환 및 최적화
        for col in ['open', 'high', 'low', 'close', 'volume']:
            if col in full_yf.columns:
                full_yf[col] = full_yf[col].astype('float32')
                
        full_yf = optimize_dtypes(full_yf)
        self.save(full_yf, "prices_kr_daily_raw")
        return full_yf
