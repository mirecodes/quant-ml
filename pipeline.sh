...
# 도움말 표시 함수
show_help() {
    echo -e "${BOLD}사용법:${NC} ./pipeline.sh [명령어] [옵션]"
    echo ""
    echo -e "${BOLD}명령어:${NC}"
    echo -e "  ${GREEN}fetch${NC}    : 데이터 및 네이버 증권 테마 수집"
    echo -e "  ${GREEN}train${NC}    : 피처 생성, 라벨 생성, TFT & LightGBM 모델 학습 및 백테스트 평가"
    ...
    echo ""
    echo -e "${BOLD}예시:${NC}"
    echo "  ./pipeline.sh fetch --no-cache --limit 100  # 100종목씩 실시간 수집"
    echo "  ./pipeline.sh train                         # 피처 빌드 및 AI 모델 학습"
    echo "  ./pipeline.sh display                       # UI 기동"
    echo "  ./pipeline.sh all --limit 30                # 30종목 제한으로 전체 파이프라인 구동"
}
...
run_train() {
    ...
    print_header "Step 3: Creating Target Labels"
    ...
    print_header "Step 4: Training TFT Model"
    $PYTHON_BIN scripts/04_train_tft.py
    if [ $? -ne 0 ]; then
        print_error "TFT 모델 학습 중 오류가 발생했습니다."
        exit 1
    fi
    print_success "TFT(Temporal Fusion Transformer) 학습 완료!"

    print_header "Step 5: Training Baseline Models"
    $PYTHON_BIN scripts/05_train_baselines.py
    if [ $? -ne 0 ]; then
        print_error "베이스라인 모델 학습 중 오류가 발생했습니다."
        exit 1
    fi
    print_success "LightGBM 및 재무 베이스라인 학습 완료!"

    print_header "Step 6: Evaluating Models"
...
run_display() {
    print_header "Step 7: Launching QuantML Dashboard"
